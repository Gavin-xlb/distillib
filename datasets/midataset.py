...
class SliceDataset(Dataset):
    def __init__(self, data_path, dataset='kits', task='tumor', series_index=None, mode='train'):
        super(SliceDataset, self).__init__()
        assert dataset in ['lits', 'kits']
        assert task in ['organ', 'tumor']
        assert mode in ['train', 'test', 'support']
        self.load_path = data_path
        self.series_index = series_index
        self.task = task
        self.mode = mode
        self.dataset = dataset
        self.tumor_slices = np.load(os.path.join(data_path, '%s_%s_slices_%s.npy' % (dataset, task, mode)))
