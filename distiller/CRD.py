...
class CRD(Distiller):
    ...
    def __init__(self, student, teacher, num_data):
    ...
    def crd_loss(self, f_s, f_t, idx):
        f_s = self.embed_s(f_s) #(batch_size,feat_dim) (16,128)
        f_t = self.embed_t(f_t) #(batch_size,feat_dim)
    ...
    def forward(self, batch, flag, support_image, support_mask, **kwargs):
        ...
        #         nn.Conv2d(high.shape[1], 256, kernel_size=1, bias=False),
        #         nn.BatchNorm2d(256),
        #     ).cuda()
        # self.align_t = nn.Sequential(
        #         nn.Conv2d(t_high.shape[1], 256, kernel_size=1, bias=False),
        #         nn.BatchNorm2d(256),
        #     ).cuda()
        ...
        loss = loss_ce * self.ce_loss_weight + loss_crd * self.feat_loss_weight
        return loss
