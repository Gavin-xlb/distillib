...
class CrossOp(nn.Module):
    ...
    def forward(self, target, support):
        ...
        new_target = interaction.mean(dim=1, keepdims=True) # B,1,H,W
...
class UniverSeg(nn.Module):
    ...
    def __post_init__(self):
        ...
        encoder_blocks = list(map(as_2tuple, self.encoder_blocks)) # 放的是cross_ch, conv_ch
...
@validate_arguments
def universeg(version: Literal["v1"] = "v1", pretrained: bool = False, num_classes=2) -> nn.Module:
    weights = {
    ...
    if version == "v1":
        ...
        model = UniverSeg(encoder_blocks=[16, 16, 16, 16], num_classes=num_classes)
...
if __name__ == '__main__':
    support_size = 64
    loadData = np.load('/data/xulingbing/projects/distillib/data/lits/tumor/slices/lits_tumor_slices_support.npy')
